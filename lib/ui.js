...
cartLineElement.dataset.productId = product.id.toString();
 ...
 function updateTotal() {
  ...
  // Loop through every product row and sum their total
  const productRows = document.querySelectorAll('tbody tr');
  productRows.forEach(row => {
      const priceCell = row.querySelector('td:nth-child(4)');
      if (priceCell) {
          const priceText = priceCell.textContent.trim().replace('ISK', '').replace(',', '').trim();
          total += parseFloat(priceText);
      }
  });
  
  // Update the Samtals total
  const samtalsElement = document.querySelector('tfoot .price');
  samtalsElement.textContent = `ISK ${total.toLocaleString('is-IS')}`;
}

// When adding a product
function addProductToCart(product, quantity) {
  // ... [your code to add the product]
  
  updateTotal();
}

// When removing a product
function deleteLineFromCart(e) {
